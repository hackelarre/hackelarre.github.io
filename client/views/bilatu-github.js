//  http://opensource.bq.com/ aterata
...
Template.lab.onRendered(function() {
  ...
  function getGithubProjects() {
    ...
    loader = $('.loader');
      ...
      $.ajax({
        type: "GET",
        url: 'https://api.github.com/orgs/hackelarre/repos?callback=?',
...
function setMarkupRepo(data) {
    ...
    projectList.append(
        ...
        +  '<span class="fa-stack fa-lg"> <i class="fa fa-star fa-stack-1x"></i></span>' + data['stargazers_count']
        +  '<span class="fa-stack fa-lg"> <i class="fa fa-code-fork fa-stack-1x"></i></span>' + data['forks_count']
        +  '</p>'
        + '</div>'
    );
}
...
function handleMixItUp() {
    ...
    languageList = ['JavaScript', 'CSS', 'C++', 'C', 'Java', 'Ruby'];
            ...
            onMixFail: function(state){
                if(state.activeFilter == 'none') {
                    ...
                    if ($('.no-match').length < 1) {
                        projectList.prepend('<p class="no-match">Ez dago ezugarri hauek dituen emaitzik</p>');   
                    }
